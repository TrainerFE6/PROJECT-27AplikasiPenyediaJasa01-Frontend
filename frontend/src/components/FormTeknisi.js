  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = formData.id_teknisi
        ? await axios.put(`https://api-msib-6-penyedia-jasa-02.educalab.id/teknisi/${formData.id_teknisi}`, data, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          })
        : await axios.post('https://api-msib-6-penyedia-jasa-02.educalab.id/teknisi', data, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            {formData.gambarURL && !formData.gambar && (
              <div>
                <img
                  src={`https://api-msib-6-penyedia-jasa-02.educalab.id/uploads/technician/${formData.gambarURL}`}
                  alt="Gambar Teknisi"
