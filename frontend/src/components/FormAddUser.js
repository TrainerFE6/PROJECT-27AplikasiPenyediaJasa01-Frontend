  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = formData.id_user
        ? await axios.put(`https://api-msib-6-penyedia-jasa-02.educalab.id/users/${formData.id_user}`, data, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          })
        : await axios.post('https://api-msib-6-penyedia-jasa-02.educalab.id/users', data, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div>
            ...
            {formData.gambarURL && !formData.gambar && (
              <div>
                <img
                  src={`https://api-msib-6-penyedia-jasa-02.educalab.id/uploads/user/${formData.gambarURL}`}
                  alt="Gambar User"
