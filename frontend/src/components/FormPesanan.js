...
const FormPesanan = ({ order, onClose }) => {
    ...
    const fetchData = async () => {
      try {
        const [adminsRes, techniciansRes] = await Promise.all([
          axios.get('https://api-msib-6-penyedia-jasa-02.educalab.id/admin'),
          axios.get('https://api-msib-6-penyedia-jasa-02.educalab.id/teknisi')
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://api-msib-6-penyedia-jasa-02.educalab.id/order/${formData.id_pesanan}`, data, {
        headers: {
